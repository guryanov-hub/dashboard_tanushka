...
- Відфільтровуються лише події з URL (через `pages[0].content_urls.desktop.page` → `titles.canonical` → `title`)
...
5. Щоб додати нову дату в UA_HISTORY — додати рядок формату `'MM-DD': [рік, 'текст', 'url']`
